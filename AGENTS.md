...
- UI text lives in `client/src/i18n/`: one exported `t({ en, fr })` constant per message, imported as `import * as m from '../i18n/<area>'`. A missing locale or unknown key fails `tsc`; an unused message fails knip. Engine text (pawns, biomes, battle log, campaign JSON) is not translated yet.
