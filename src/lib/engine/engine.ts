...
export function targetingTiles(state: GameState): Set<string> {
  ...
  if (state.phase === 'special') {
    return new Set(pawn.specialTiles(state).map((tile) => key(tile.q, tile.r)))
  }
  const targets =
    state.phase === 'attack'
...
function executeAction(state: GameState, action: Action): ActionResult | null {
  ...
  switch (action.type) {
    case 'act':
    case 'specialAt': {
      if (action.type === 'act' && action.action !== 'special') return null
      if ((action.type === 'act') !== (actor.specialPhase === 'move')) return null
      if (state.phase !== actor.specialPhase) return null
      const result = actor.performSpecial(
        { tiles, pawns, round: state.round, log, random },
        action.type === 'act' ? from : action,
      ...
      effect = {
        ...
        from,
        to: result.to,
        ...('impacts' in result ? { impacts: result.impacts } : {}),
      }
      break
    }
    case 'move': {
...
function reduce(
  ...
  switch (action.type) {
    ...
    case 'act':
      ...
      if (action.action === 'special' && pawn.specialPhase !== 'move')
        return canUseSpecial(pawn) ? { ...state, phase: 'special' } : state
      break
    case 'specialAt':
      if (state.phase === 'special' && pawn.specialPhase === 'charge') {
